...
export function RFQForm() {
  ...
  return <form className="grid gap-6 border border-border bg-surface p-6 sm:p-8" onSubmit={submit}>
    ...
    <label className="group flex min-h-28 cursor-pointer items-center gap-4 border border-dashed border-border bg-surface-2 px-5 py-6 text-sm text-muted transition hover:border-accent focus-within:border-accent"><span className="grid size-11 place-items-center border border-border transition group-hover:border-accent"><Upload className="size-5 text-accent" /></span><span>Attach RFQ, BOQ, datasheet, drawing or photograph<span className="mt-1 block text-xs">PDF, JPG, PNG, DOC, DOCX, XLS or XLSX · 10 MB maximum</span></span><input className="sr-only" type="file" name="attachment" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx" /></label>
